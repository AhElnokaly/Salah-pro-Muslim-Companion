...
export default function UnifiedProgressCard({
  ...
  return (
    <div id="unified-progress-section" className="w-full space-y-4 font-sans" dir="rtl">
      ...
      <div className={`rounded-3xl p-3 sm:p-4 border transition-all duration-300 shadow-md relative overflow-hidden ${
        isDark
        ...
        <div className="flex items-center justify-between gap-2 pb-2.5 border-b border-slate-200/50 dark:border-slate-800/80">
          <div className="flex items-center gap-2 min-w-0">
            <div className={`w-7.5 h-7.5 sm:w-8.5 sm:h-8.5 rounded-xl flex items-center justify-center text-base sm:text-lg shadow-2xs shrink-0 ${
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                <h3 className="text-xs sm:text-sm font-black text-slate-900 dark:text-white leading-none truncate">
                  مؤشر الإتقان الإيماني
                </h3>
                {isWomenExcuse && (
                  <span className="text-[8.5px] px-1.5 py-0.2 rounded-full bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-300 font-extrabold shrink-0">
                    عذر شرعي 🌸
                  </span>
                )}
              </div>
          ...
          <button
            ...
            className={`px-2 py-1 rounded-xl text-[10px] sm:text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 shrink-0 hover:scale-105 active:scale-95 ${
              dailyProgress.overallTier.colorClasses.badgeBg
          ...
          >
            ...
            <span className="truncate max-w-[85px] sm:max-w-none">{dailyProgress.overallTier.title}</span>
            <span className="text-[9px] bg-black/20 text-white px-1.5 py-0.2 rounded-md font-mono">
              {toArabicNumbers(dailyProgress.overallPercentage)}%
            </span>
          </button>
        </div>

        {/* Period Selector Tabs & Legend (Unified in one compact line) */}
        <div className="flex items-center justify-between gap-1.5 pt-2 pb-0.5">
          <div role="tablist" aria-label="فترة الإنجاز" className="flex items-center gap-0.5 bg-slate-100/90 dark:bg-slate-800/80 p-0.5 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
            <button
              type="button"
              role="tab"
              ...
              className={`px-2 py-0.5 rounded-lg text-[10px] sm:text-xs font-black transition-all cursor-pointer flex items-center gap-1 ${
                activePeriod === 'daily'
          ...
          <div className="flex items-center gap-1.5 text-[8.5px] sm:text-[9.5px] font-bold px-2 py-0.5 rounded-lg bg-slate-100/70 dark:bg-slate-800/50 border border-slate-200/50 dark:border-slate-700/50 text-slate-500 dark:text-slate-400 shrink-0">
            <span className="flex items-center gap-0.5 text-emerald-600 dark:text-emerald-400 font-extrabold">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0" />
              حاضر
            </span>
            <span>•</span>
            <span className="flex items-center gap-0.5 text-amber-600 dark:text-amber-400 font-extrabold">
